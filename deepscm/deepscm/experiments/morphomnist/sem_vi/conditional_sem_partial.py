...
class ConditionalVISEM_partial(BaseVISEM):
    context_dim = 2
    ...
    def pgm_model(self):
        ...
        thickness = pyro.sample('thickness', thickness_base_dist)
        ...
        intensity = pyro.sample('intensity', intensity_base_dist)
    ...
    def model(self):
        ...
        latent = torch.cat([z, thickness, intensity], 1)
    ...
    def guide(self, x, thickness, intensity):
        with pyro.plate('observations', x.shape[0]):
            ...
            hidden = torch.cat([hidden, thickness, intensity], 1)
...
MODEL_REGISTRY[ConditionalVISEM_partial.__name__] = ConditionalVISEM_partial
