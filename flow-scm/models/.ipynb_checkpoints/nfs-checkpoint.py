...
#         self.risk_flow_transforms = [self.risk_flow_component,self.risk_flow_constraint ]
    ...
    def pgm_model(self):
        ...
        #print('AGE----',self.age,self.age.shape)
...
def _conditionalscm(arch, flow_dict, sex_dim, flow_type, bins, order):
    model = ConditionalSCM(flow_dict=flow_dict, sex_dim=sex_dim,
        flow_type=flow_type, spline_bins=bins, spline_order=order)
    return model

def conditionalscm(flow_dict=None, sex_dim=1, flow_type='affine', bins=8, order='linear'):
    return _conditionalscm('conditionalscm', flow_dict, sex_dim, flow_type, bins, order)
